...
def getauthToken(appId, redirect_uri):
    ...
    appSession = accessToken.SessionModel(client_id=appId,redirect_uri=redirect_uri,response_type=response_type, grant_type=grant_type,state="state",scope="",nonce="")
...
def main():
    ...
    auth_code = input ("Enter auth value: ")

    # Function to get the access token and need to be commented while calling the getauthToken(x,x) func.
    token_response = generate_access_token(auth_code, appId, app_secret)

    accessToken = token_response["access_token"]


def MapOrderToFyers(orderRequest):
